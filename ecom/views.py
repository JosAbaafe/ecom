...
def admin_dashboard_view(request):
    ...
    ordertoday = models.Orders.objects.filter(order_date=today).count()
    print(ordertoday)
    # for t in transac:
    #     total += t.price()
    ...
    sorted_orders=models.Orders.objects.all().order_by('-order_date')
    orders=models.Orders.objects.all().filter(order_date=today)
    total = 0
    for a in orders:
    ...
    for order in orders:
        ...
        # order_today = orders.objects.filter(order_date=today).count()
        # print(order_today)
...
def payment_success_view(request):
    ...
    if 'product_ids' in request.COOKIES:
        ...
        # pro = product.objects.get(id=product_ids)
    ...
    for product in products:
        ...
        pro = models.Product.objects.filter(id = product.id)
        for p in pro:
